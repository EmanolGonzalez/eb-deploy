...
# =============================================================================
# deploy.sh — Instalación/actualización unificada de componente
# Modo install: comportamiento de install.sh
# Modo update : comportamiento de update.sh (healthcheck estricto + auto-rollback)
# =============================================================================
...
resolve_mode() {
  ...
  if [[ -n "${DEPLOY_MODE:-}" ]]; then
    MODE="$DEPLOY_MODE"
  elif [[ "$script_name" == "install.sh" ]]; then
    MODE="install"
  elif [[ "$script_name" == "update.sh" ]]; then
    MODE="update"
  elif [[ "$mode_arg" == "install" || "$mode_arg" == "update" ]]; then
    MODE="$mode_arg"
    MODE_ARG_CONSUMED=true
  else
    arrow_select "Selecciona operación:" "install" "update"
    MODE="$ARROW_SELECTION"
  fi

  if [[ "$MODE" != "install" && "$MODE" != "update" ]]; then
    err "Modo inválido: $MODE (usa 'install' o 'update')."
    exit 1
  fi
...
validate_download_client() {
  ...
  err "Instala curl o wget para usar despliegue por URL directa."
  exit 1
}
...
select_component() {
  local prompt="Selecciona el componente a ${OP_LABEL}:"
  arrow_select "$prompt" frontend backend
...
main() {
  resolve_mode "${1:-}"
  if [[ "$MODE_ARG_CONSUMED" == true ]]; then
    shift
  fi
  ...
  if [[ "$MODE" == "update" ]]; then
    run_healthcheck_or_rollback
    log "Actualización de $COMPONENT a v${VERSION} completada exitosamente (origen: $SOURCE_MODE)."
  else
